...
`previewDryRun()` follows fixture links without transport or persistence writes. It reports unique pre-backfill URLs, distinct box-score links, unavailable target coverage, page-type counts, and a minimum runtime estimate based on the configured request interval. Live PostgreSQL migration execution is recorded in `FOUNDATION_HANDOFF.md`.
